...
parser.add_argument("--amp", default=True, help="do NOT use amp for training")
parser.add_argument("--val_every", default=2, type=int, help="validation frequency")
...
parser.add_argument("--space_x", default=1.4, type=float, help="spacing in x direction") 
parser.add_argument("--space_y", default=1.4, type=float, help="spacing in y direction") 
...
model =SwinVnet(img_size=(args.roi_x, args.roi_y, args.roi_z),
...
model_inferer = partial(sliding_window_inference,roi_size=inf_size,sw_batch_size=args.sw_batch_size,predictor=model,overlap=args.infer_overlap)
...
information_line ='start train'+ '\n'
open('Log.txt', 'w').write(information_line)  #向txt文件中写入
